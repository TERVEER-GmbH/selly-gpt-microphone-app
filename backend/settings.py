    ...
    def deserialize_tools(cls, tools_json_str: str) -> List[_AzureOpenAITool]:
        if isinstance(tools_json_str, str):
            try:
                ...
                logger.warning("No valid tool definition found in the environment.  If you believe this to be in error, please check that the value of AZURE_OPENAI_TOOLS is a valid JSON string.")

            except ValidationError as e:
                logger.warning(f"An error occurred while deserializing the tool definition - {str(e)}")
    ...
    def deserialize_logit_bias(cls, logit_bias_json_str: str) -> dict:
        if isinstance(logit_bias_json_str, str):
            try:
                ...
                logger.warning(f"An error occurred while deserializing the logit bias string -- {str(e)}")
...
class _AzureSearchSettings(BaseSettings, DatasourcePayloadConstructor):
    ...
    def _set_filter_string(self, request: Request) -> str:
        if self.permitted_groups_column:
            ...
            logger.debug(f"USER TOKEN is {'present' if user_token else 'not present'}")
            if not user_token:
                raise ValueError(
    ...
    def set_datasource_settings(self) -> Self:
        try:
            if self.base_settings.datasource_type == "AzureCognitiveSearch":
                ...
                logger.debug("Using Mongo DB")

            else:
                self.datasource = None
                logger.warning("No datasource configuration found in the environment -- calls will be made to Azure OpenAI without grounding data.")
